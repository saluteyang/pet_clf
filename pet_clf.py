...
logitCV = LogisticRegressionCV(cv=5, random_state=0, scoring='f1_macro').fit(X_train, label_train)
logitCV.score(X_train, label_train)   # 0.77 (acc); 0.636 (acc, f1_macro scoring)
logitCV.score(X_test, label_test)   # 0.74 (acc); 0.598 (acc, f1_macro scoring)

f1_score(label_train, logitCV.predict(X_train))   # 0.41
f1_score(label_test, logitCV.predict(X_test))   # 0.36
